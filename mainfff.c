...
double calcalt(double press, double temp)
{
	const float p0 = 101325;     // Pressure at sea level (Pa)
	double alt=0;
	alt = (double)44330 * (1 - pow(((double) press/p0), 0.190295));
...
void graph(uint16_t value)
{
	...
	static uint16_t posy_alt=0;
	static uint32_t correction=96200;
	
	ili9341_drawLine(posx-1, posy_alt-correction, posx, posy-correction, YELLOW);
	posx++;
}
int main(void)
{
	...
	DPS310_init(HIGH);
	_delay_ms(100);
	if(TEMP_READY_CHECK)tt=DPS310_get_temp(temp_ovs);
	if(PRES_READY_CHECK)pp=DPS310_get_pres(temp_ovs, pres_ovs);
	while(1)
	{
		...
		altitude = calcalt(Pressure, Temperature);//(long) 44330 * (1 - pow(((long) Pressure/101325), 0.190295));
		
		ili9341_setcursor(0,30);
		printf("Altidude: %ld", altitude);
