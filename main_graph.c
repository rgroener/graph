...
uint8_t xtime, temp_new, temp_old, pres_new, pres_old;
...
int main(void)
{
	...
	_delay_ms(200);
	temp_new=Temperature=DPS310_get_temp(temp_oversampling);
	temp_old=temp_new;
	pres_new=Pressure=DPS310_get_pres(temp_oversampling, prs_oversampling);
	pres_old=pres_new;
	...
	while(1)
	{
		...
		if(TEMP_READY_CHECK && measurement)
		{
			measurement=0;
			Temperature=DPS310_get_temp(temp_oversampling)/2;
			temp_new=2800-Temperature;
			xtime++;
			if(xtime>320)
			{
				xtime=0;
			...
			ili9341_drawLine(xtime-1, temp_old, xtime, temp_new, YELLOW);
			temp_old=temp_new;
			
			ili9341_setcursor(10,0);
			printf("T: %d.%1.2d", vor_komma(Temperature), nach_komma(Temperature));
		}
